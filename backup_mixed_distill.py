...
class CustomDistillationTrainer(Trainer):
    def __init__(self, mlp_forget_model, *args, **kwargs):
        """
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if is_forget.any():
            ...
            forget_loss_alpha = (forget_teacher_probs.max(dim=-1).values > 0.5).float().mean().item()
            forget_loss_alpha = max(0.8, loss_alpha) #0.7
            ...
            total_loss += forget_loss_alpha*forget_loss
